...
export default function Footer() {
  ...
  const handleSubscribe = async (e) => {
    ...
    try { const { data } = await axiosInstance.post("/support/subscribe", { email }); toast.success(data.message); setEmail(""); }
    catch (error) { toast.error(error.response?.data?.message || "Unable to subscribe right now"); }
  ...
  return <footer className="bg-stone-900 text-stone-300 pt-12">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      ...
      <nav aria-label="Customer Service"><h3 className="text-lg font-bold text-yellow-400 mb-4">Customer Service</h3><ul className="text-sm space-y-2"><li><Link to="/contact" className="hover:underline">Contact Us</Link></li><li><Link to="/faq" className="hover:underline">FAQ</Link></li><li><Link to="/shipping-returns" className="hover:underline">Shipping & Returns</Link></li><li><Link to="/support" className="hover:underline">Support</Link></li></ul></nav>
