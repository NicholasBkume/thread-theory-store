...
export const useOrderStore = create((set) => ({
    ...
    cancelOrder: async (orderId) => { try { const { data } = await axios.post(`/orders/${orderId}/cancel`); set((state) => ({ orders: state.orders.map((order) => order._id === orderId ? data.order : order) })); toast.success(data.message); return true; } catch (error) { toast.error(error.response?.data?.message || "Failed to cancel order"); return false; } },
}));
