...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        if (productMap.size !== uniqueProductIds.length) throw new Error("One or more products are no longer available");
        ...
        const lineItems = requestedProducts.map(({ productId, quantity }) => {
            ...
            if (!Number.isFinite(price) || price <= 0) throw new Error(`Product ${productId} has an invalid price`);
            return {
                price_data: {
                    currency: "usd",
                    product_data: { name: product.name, ...(product.image ? { images: [product.image] } : {}) },
        ...
        const sessionParams = {
            ...
            metadata: {
                ...
                products: JSON.stringify(requestedProducts.map(({ productId, quantity }) => ({
                    id: productId,
                    quantity,
        ...
        return res.status(400).json({ message: error?.message || "Error processing checkout" });
    }
};
...
export const checkoutSuccess = async (req, res) => {
    try {
        ...
        if (!order) return res.status(400).json({ message: "Payment has not been completed." });
        return res.status(200).json({ success: true, message: "Payment successful, order created, and coupon deactivated if used.", orderId: order._id });
    } catch (error) {
        console.error("Error processing successful checkout:", error);
        return res.status(500).json({ message: error?.message || "Error processing successful checkout" });
    }
};
...
export async function finalizePaidCheckout(session) {
    ...
    if (!Array.isArray(products) || products.length === 0) throw new Error("Stripe session contains no order products");

    const quantityByProduct = new Map();
    for (const item of products) {
        if (!mongoose.isValidObjectId(item.id) || !Number.isInteger(item.quantity) || item.quantity < 1) {
            throw new Error("Stripe session contains invalid product metadata");
        }
        quantityByProduct.set(item.id, (quantityByProduct.get(item.id) || 0) + item.quantity);
    }

    // Atomically reserve inventory. A matching stock condition prevents two
    // concurrent paid webhooks from consuming the same units.
    for (const [productId, quantity] of quantityByProduct) {
    ...
    try {
        ...
        // A unique Stripe session id makes duplicate webhook deliveries safe.
        // If order creation fails for another reason, restore the inventory.
        if (error?.code !== 11000) {
            for (const [productId, quantity] of quantityByProduct) {
